<?php
    ...
    class Funcionario extends Model{
        ...
        public function read(){
            $query = 'SELECT f.id, f.email, f.nome, f.telefone, f.turnos, s.setor FROM funcionarios AS f JOIN setores AS s ON f.id_setor = s.id WHERE f.situacao = 1 ORDER BY f.id DESC';
            $stmt = $this->db->prepare($query);
            $stmt->execute();
