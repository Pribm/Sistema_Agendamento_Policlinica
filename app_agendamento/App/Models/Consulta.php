<?php
    ...
    class Consulta extends Model{
        ...
        public function agendaPaciente(){
            $query = 'INSERT INTO agendamentos (paciente, medico_id, prontuario, sus, horario, dia, agendado_por) VALUES (:paciente, :medico_id, :prontuario, :sus, :horario, :dia, :agendado_por)';
        ...
        public function filtrar(){
            $query = "SELECT a.paciente, a.id, m.nome AS 'medico', a.prontuario, f.nome AS 'solicitante',a.atendimento_realizado, a.horario, DATE_FORMAT(a.dia, '%d/%m/%y') AS 'dia' FROM agendamentos AS a JOIN funcionarios AS m ON a.medico_id = m.id JOIN funcionarios AS f ON a.agendado_por = f.id WHERE (m.id = :medico OR :medico = '') AND (a.dia = :dia OR :dia = '') AND (a.prontuario = :prontuario OR :prontuario = '') AND (a.horario = :horario OR :horario = '') AND (a.paciente LIKE CONCAT('%', :paciente, '%') OR :paciente = '')";
            $stmt = $this->db->prepare($query);
